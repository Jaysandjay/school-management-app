...
export default function DeleteModal ({id, type, isOpen, onClose}: DeleteModalProps) {
    ...
    function getMutation(id: number){
        switch(type){
        case "student": return   deleteStudent(id)
        case "teacher": return   deleteTeacher(id)
        case "guardian": return  deleteGuardian(id)
        case "class": return     deleteClass(id)
        }
    }
    ...
    onSuccess: () => {
        queryClient.invalidateQueries({queryKey: [type == "class" ? type + "es" : type + "s"]})
        }
    })
        ...
        <BasicModalContainer >
            ...
            <div>
                ...
                <PrimaryButton
                    ...
                    onclick={() => {
                        onClose()
                        router.push(type == "class" ? `/${type}es` : `/${type}s`)
                        setIsDeleting(false)
                    }}
                />
                ) : (
